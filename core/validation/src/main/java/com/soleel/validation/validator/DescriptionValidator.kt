...
class DescriptionValidator : InValidation<String, ResultValidation> {
    override fun execute(input: String): ResultValidation {
        ...
        if (input.length < 12) {
            return ResultValidation(
                successful = false,
                errorMessage = UiText.StringResource(resId = R.string.description_too_short_error_message)
            )
        }
        ...
        if (input.length > 64) {
            return ResultValidation(
                successful = false,
                errorMessage = UiText.StringResource(resId = R.string.description_too_long_error_message)
            )
        }
